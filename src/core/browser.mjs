...
export async function launchBrowser({
  ...
  const stdoutFd = artifactStore.openLog('chrome-stdout.log');
  const stderrFd = artifactStore.openLog('chrome-stderr.log');
  const args = [
    '--no-sandbox', '--no-first-run', '--no-default-browser-check', '--remote-debugging-port=0',
  ...
  const child = spawn(chromeBin, args, { stdio: ['ignore', stdoutFd, stderrFd], env: process.env });
  ...
  const browserCdp = new CDP(version.webSocketDebuggerUrl, onHandlerError);
  await browserCdp.connect();
  const page = await waitFor(async () => {
    const targets = await fetch(`http://127.0.0.1:${port}/json/list`).then((response) => response.json());
    return targets.find((target) => target.type === 'page');
  }, { timeoutMs: 10_000, description: 'Chrome page target' });
  const pageCdp = new CDP(page.webSocketDebuggerUrl, onHandlerError);
